...
function mtTable.__index:SetMetatable(mtVal)
	if type(mtVal) == "table" then
		...
		return debug.setmetatable(self, mtVal)
	end
end
...
function Table.Apply(tb)
	if tb == (_ENV or _G) then
		Table.Apply(debug.getmetatable(""))
		Table.Apply(debug.getmetatable(io.stdin))
		
		local mtClone = mtTable:Clone()
		function mtClone:__newindex(key1, val1)
			if type(key1) == "table" and not debug.getmetatable(key1) then
				setmetatable(key1, mtClone)
			end
			if type(val1) == "table" and not debug.getmetatable(val1) then
